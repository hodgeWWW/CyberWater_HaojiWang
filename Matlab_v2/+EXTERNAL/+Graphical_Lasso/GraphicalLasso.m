...
if nargin < 8
    tolThreshold = 1e-6;  %改：tolThreshold = 1e-4;  
end
if nargin < 9
    maxIter = 1e4;
end
...
[w, theta, iter, avgTol, hasError] = EXTERNAL.Graphical_Lasso.glasso(numVars, S, 0, lambda, approximate, warmInit, verbose, penalDiag, tolThreshold, maxIter, w, theta);
toc
%[w, theta, iter, avgTol, hasError] = MyGlassoMex.glasso(numVars, S, lambda, approximate, warmInit, verbose, penalDiag, tolThreshold, maxIter, w, theta);
%[w, theta, iter, avgTol, hasError] = MyAutoGlasso.glasso(numVars, S, lambda, approximate, warmInit, verbose, penalDiag, tolThreshold, maxIter, w, theta);
